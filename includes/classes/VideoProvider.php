...
class Videoprovider{
  public static function getUpNext($con, $currentVideo){
      ...
      if($query->rowCount() == 0)
      {
        $query = $con->prepare("SELECT * FROM videos WHERE season<=1 && episode<=1 AND id!=:videoId
                                ORDER BY views DESC LIMIT 1");  //if season 0 then its a movie
        $query->bindValue(":videoId", $currentVideo->getId());
        $query->execute();
      }
  ...
  public static function getEntityVideoForUser($con, $entityId, $username){
    ...
    if($query->rowCount() == 0)  //if not seen this show before
    {
